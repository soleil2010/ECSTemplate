...
//    time.
// -> server-builds still create the client world for convenience, but don't
//    update it
using System;
...
namespace DOTSNET
{
    public class Bootstrap : ICustomBootstrap
    {
        ...
        // helper function to prepend systems that need OnCreate called before
        // all other systems
        static void PrependSetupSystems(List<Type> systemTypes)
        {
            // need to inject dependencies before all other system's OnCreate,
            // so that their fields are available in OnCreate already.
        ...
        public static World CreateDefaultWorld(string defaultWorldName, List<Type> unitySystems, List<Type> regularSystems)
        {
            ...
            // prepend dependency injection etc.
            PrependSetupSystems(regularSystems);

            // add systems to root level system group
        ...
        public static World CreateServerWorld(string worldName, List<Type> unitySystems, List<Type> serverSystems, World defaultWorld, bool addToDefaultUpdateList)
        {
            ...
            // prepend dependency injection etc.
            PrependSetupSystems(serverSystems);
        ...
        public static World CreateClientWorld(string worldName, List<Type> unitySystems, List<Type> clientSystems, World defaultWorld, bool addToDefaultUpdateList)
        {
            ...
            // prepend dependency injection etc.
            PrependSetupSystems(clientSystems);
            ...
            // add hooks in DefaultWorld to update ClientWorld
            // server-only builds don't update client world for performance.
            // never creating it would work too, but then we'd need UNITY_SERVER
            // checks all over DOTSNET because lots of scripts access
            // ClientWorld.
            // => for example, authoring scripts access it once in awake
            // => not updating is the most simple solution
#if !UNITY_SERVER
            if (addToDefaultUpdateList)
            {
